...
try:
    ...
    Emailfrom = "CIOCHINA.Automation@accenture.com"
    Emailto = configs["emailToList"]
    Emailcc = configs["emailCcList"]
    failEmailToList = configs["failEmailToList"]
    failEmailCcList = configs["failEmailCcList"]

    #创建当日截图文件夹
    screenshotPath = r'.\Screenshot'+timenow
    fullPath = os.path.abspath(screenshotPath)
    setscreenshotfile(screenshotPath)
    ...
    selectedProjects = configs["selectedProjects"]
    ...
    #  DeliverService
    driver.get("https://bi6.accenture.com/#/views/DeliverService_all/DeliverService/shaojun.qu@dir.svc.accenture.com/DeliveryService-CAMSMyCompetency?:iid=1")
...
#Emil section
    ...
    msgRoot['From'] = Header(Emailfrom, 'utf-8')
    ...
    HTMLBody = '''
        <p>Hi All,</p>
        <p>Please get below screen shot and table of CAMS Team CIO Now report on '''+timenow+'''.</p>
        <h3>Compliance</h3>
...
# add png
    addpng(screenshotPath + r'\ComplianceOps_cropped.png', '<Compliance>')
    addpng(screenshotPath + r'\PlanAndManageService.png', '<PlanAndManageService>')
    addpng(screenshotPath + r'\Deliver Service.png', '<DeliverService>')
